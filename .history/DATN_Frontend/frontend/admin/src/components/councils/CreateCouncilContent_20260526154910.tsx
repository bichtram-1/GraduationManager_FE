              ...
              <div>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-gray-500">Chọn nhiều GVHD và nhiều GVPB. Không còn vai trò chủ tịch / thư ký.</div>
                  </div>
                  <div className="rounded-full bg-gray-100 px-3 py-1 text-[11px] text-gray-600">
                    {advisorIds.length} GVHD · {reviewerIds.length} GVPB
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-xl border border-gray-200">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      <tr>
                        <td className="px-4 py-3 font-medium">Giảng viên hướng dẫn</td>
                        ...
                        <td className="px-4 py-3 text-xs text-gray-500">GVHD quyết định nhóm đề tài nào được gom vào hội đồng</td>
                      </tr>
                      <tr>
                        ...
                        <td className="px-4 py-3 text-xs text-gray-500">GVPB dùng để phân công phản biện cho đề tài đã chọn</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
        ...
        <div className="space-y-6">
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 bg-gray-50 px-5 py-4">
              <div>
                <div className="font-medium">PHẦN 2: Chọn đề tài theo giảng viên hướng dẫn</div>
                <div className="mt-1 text-xs text-gray-500">Tick 1 vài đề tài trong từng GVHD, sau đó sắp xếp phần đã chọn theo thứ tự bảo vệ.</div>
              </div>
              <div className="rounded-full bg-gray-100 px-3 py-1 text-[11px] text-gray-600">Đã chọn {selectedTopics.length} đề tài</div>
            </div>

            <div className="space-y-4 p-5">
              {ADVISOR_BUCKETS.map((bucket) => {
                const isActiveAdvisor = advisorIds.includes(bucket.advisorId);
                const selectedInBucket = selectedCountByAdvisor(bucket.advisorId);

                return (
                  <div key={bucket.advisorId} className={`overflow-hidden rounded-xl border ${isActiveAdvisor ? 'border-gray-200' : 'border-dashed border-gray-200 opacity-70'}`}>
                    <div className="flex flex-wrap items-center justify-between gap-3 bg-gray-50 px-4 py-3">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-gray-500">
                          ...
                          {isActiveAdvisor ? '' : ' · chọn GVHD này ở bảng bên trái để mở đề tài'}
                        </div>
                      </div>
                      <div className="rounded-full bg-white px-3 py-1 text-[11px] text-gray-600">{isActiveAdvisor ? 'Đang hoạt động' : 'Chưa chọn'}</div>
                    </div>
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 bg-gray-50 px-5 py-4">
              <div>
                <div className="font-medium">PHẦN 3: Sắp xếp đề tài đã chọn</div>
                <div className="mt-1 text-xs text-gray-500">Kéo thả để đổi thứ tự bảo vệ. GVPB được chọn từ danh sách phản biện đã thêm ở trên.</div>
              </div>
            </div>

            {selectedTopics.length === 0 ? (
              <div className="px-5 py-16 text-center text-gray-500">Chưa có đề tài nào. Tick đề tài ở bảng GVHD bên trái để đưa vào danh sách sắp xếp.</div>
            ) : (
