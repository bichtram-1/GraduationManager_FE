...
import { cn, NotAvailable } from '@shared/constants/commonConst';
...
const StatisticCard = ({
  ...
  const getColorThemeClass = (theme: StatisticCardProps['colorTheme']) => {
    switch (theme) {
      case 'green':
      ...
      <Flex vertical gap={4}>
        <Flex align="center" gap={16}>
          ...
          <Flex vertical justify="space-between" className="flex-1">
            <Text>{title ?? NotAvailable}</Text>
            <Text className={cn('font-medium', classNameCustom)}>
              {value ?? NotAvailable}
            </Text>
          </Flex>
        </Flex>
