  ...
  return (
    <div className="council-ui min-h-screen bg-[#f5f4f0] text-[#1a1916]">
      <style>{`
        ...
        .council-ui .council-body { display:grid; grid-template-columns:repeat(4, minmax(0, 1fr)); gap:12px; border-top:1px solid var(--color-border-tertiary); padding-top:12px }
        .council-ui .role-col { min-width:0; display:flex; flex-direction:column; gap:8px; padding:0 2px }
        ...
        @media (max-width:900px){ .council-ui .council-body{grid-template-columns:1fr 1fr} }
        @media (max-width:700px){ .council-ui .stats{grid-template-columns:1fr 1fr} .council-ui .council-body{grid-template-columns:1fr} }
      `}</style>
              ...
              <div className="council-body">
                ...
                <div className="role-col">
                  ...
                  <div style={{ display: 'flex', flexWrap: 'nowrap', gap: 8, alignItems: 'center', overflowX: 'auto' }}>
                    ...
                    {/* Inline toggle for adding external lecturer, now on the same row as Xóa */}
                    <button
                      className="btn btns btn-xs"
                      onClick={() => setExternalFormCouncilId((cur) => (cur === c.id ? null : c.id))}
                    >
                      {externalFormCouncilId === c.id ? 'Đóng' : 'Thêm GV ngoài'}
                    </button>
                  </div>
