  ...
  return (
    <div className="council-ui min-h-screen bg-[#f5f4f0] text-[#1a1916]">
      <style>{`
        ...
        .council-ui .ov-head { display:flex; justify-content:space-between; align-items:center; gap:8px }
        .council-ui .chip { display:inline-flex; padding:4px 8px; border-radius:16px; background: #f3f5f7; font-size:12px }
          ...
          {COUNCILS.map((c) => (
            <div className="ov-card" key={c.id}>
              <div className="ov-head">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>GVHD · GVPB</div>
                    <div style={{ marginTop: 6 }}>
                      {c.chair.map((t) => <span key={t} className="chip" style={{ marginRight: 6 }}>{t}</span>)}
                      {c.reviewer.map((t) => <span key={t} className="chip" style={{ marginRight: 6 }}>{t}</span>)}
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' }}>
                    <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>GV ngoài</div>
                    {c.external?.length ? (
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <span className="chip">{c.external[0].name} ({c.external[0].council})</span>
                        <button className="btn btns btn-xs" onClick={() => message.info('Màn hình mẫu')}>Xóa</button>
                      </div>
                    ) : (
                      <div style={{ fontSize: 12, color: 'var(--color-text-secondary)' }}>Chưa có</div>
                    )}
                    <div>
                      <button
                        className="btn btns btn-xs"
                        onClick={() => setExternalFormCouncilId((cur) => (cur === c.id ? null : c.id))}
                      >Thêm GV ngoài</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {externalFormCouncilId === c.id && (
                <div style={{ marginTop: 10, padding: 12, borderRadius: 8, background: '#fafafa' }}>
                  <div style={{ marginBottom: 8, color: 'var(--color-text-secondary)' }}>Chọn giảng viên chấm chéo</div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <select style={{ flex: 1 }} defaultValue="TS. Lý Văn G (HĐ2)">
