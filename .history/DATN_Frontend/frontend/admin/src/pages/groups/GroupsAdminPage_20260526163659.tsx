...
const GroupsAdminPage: React.FC = () => {
  ...
  const columns = [
    ...
    { title: 'Số thành viên', key: 'count', render: (_: any, r: Group) => `${r.members.length}/${r.maxMembers}` },
    {
      ...
      render: (_: any, r: Group) => (
        <Space>
          <Button size="small" onClick={() => openDrawer(r)}>Xem / Sửa</Button>
          <Button size="small" onClick={() => openAddMember(r)}>Thêm thành viên</Button>
          <Button size="small" onClick={() => { setMergeMode(true); setMergeLeft(r.id); }}>
            Ghép</Button>
          <Button size="small" danger onClick={() => promptDelete(r)}>Xóa</Button>
        </Space>
      ),
    },
  ];
              ...
              <div className="flex flex-col gap-2">
                {drawerGroup.members.map((m) => (
                  <div key={m.id} className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      {!m.eligible && <Button danger size="small" onClick={() => removeMember(drawerGroup.id, m.id)}>Xóa</Button>}
                      {!m.eligible && <Button size="small" onClick={() => { setGroups((prev) => prev.map((g) => g.id === drawerGroup.id ? { ...g, status: 'LOCKED' } : g)); message.info('Đã khóa nhóm (mô phỏng)'); }}>Khóa nhóm</Button>}
                    </div>
                  </div>
                ))}
              </div>
