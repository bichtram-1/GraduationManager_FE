...
import { Button, Card, Form, Input, Modal, Pagination, Select, Space, Tag, Typography, message, Tabs } from 'antd';
  ...
  const submitTopicForm = async () => {
    try {
      ...
      if (topicModalMode === 'edit' && selectedTopic) {
        setTopicRows((prev) =>
      render: (_status: TopicStatus, record) => {
        ...
        const canReject = record.status !== 'rejected';
          ...
          <div className="flex flex-col gap-2">
            <Tag style={{ margin: 0, borderRadius: 999, padding: '0 10px', border: 'none', backgroundColor: meta.bg, color: meta.color, width: 'fit-content' }}>
              {meta.label}
            </Tag>
            <Space size={4} wrap>
              {canApprove && (
              ...
              {record.status !== 'pending' && (
                <Button
                  ...
                  onClick={() => confirmTopicChange(record, 'pending')}
                >
                  Về chờ duyệt
                </Button>
              )}
