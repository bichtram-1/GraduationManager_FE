              ...
              <Radio.Group value={selectedTeacher} onChange={(e) => setSelectedTeacher(e.target.value)} className="w-full">
                <div className="space-y-3">
                  {filteredTeachers.map((teacher) => (
                    <div key={teacher.id} className="flex items-center justify-between border border-slate-100 p-3 rounded-md">
                      <div className="flex items-center gap-3">
                        <Radio value={teacher.id} />
                        <div>
                          <div className="font-medium">{teacher.name}</div>
                          <div className="text-xs text-slate-500">{teacher.degree} • {teacher.major}</div>
                        </div>
                      </div>
                      <div>
