...
n  const summary = [
    ...
    { title: 'Trễ hạn', value: TTTN.filter((item) => item.status === 'Trễ hạn').length.toString(), hint: 'Cần gửi nhắc nộp bài' },
  ]

  const selectedCount = segment === 'TTTN' ? filteredTTTN.length : filteredDATN.length
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Khối này nhấn mạnh số lượng, trạng thái và hành động ngắn: mở chi tiết, Nhận xét, hoặc nhắc nộp đúng nhịp.
            </p>
          </div>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                ...
                <div className="mt-2 rounded-md border border-slate-100 bg-white p-4 text-sm text-slate-700">
                  ...
                  <div className="mt-2 text-sm text-slate-600">Mô tả tóm tắt nội dung báo cáo, công việc đã thực hiện và kết quả đạt được. Nội dung này là ví dụ để hiển thị trong modal xem báo cáo.</div>
                </div>
              </div>
...
const TTTN = [
  { id: '20520001', name: 'Nguyễn Văn A', company: 'FPT Software', mentor: 'TS. Nguyễn Văn X', phone: '0901 234 567', email: 'xnv@fpt.com', report: 'Báo cáo Tuần 3', status: 'Đã nộp', date: '15/05/2026' },
  { id: '20520002', name: 'Trần Thị B', company: 'VNG Corp', mentor: 'ThS. Lê Thị Y', phone: '0909 111 222', email: 'ylt@vng.com', report: 'Báo cáo Tuần 2', status: 'Trễ hạn', date: '08/05/2026' },
  { id: '20520003', name: 'Phạm Văn C', company: 'TMA Solutions', mentor: 'TS. Trần Văn Z', phone: '0933 444 555', email: 'ztv@tma.com', report: 'Báo cáo Tuần 3', status: 'Đã nộp', date: '16/05/2026' },
]

const DATN = [
  { group: 'G01', topic: 'Hệ thống quản lý thư viện điện tử', members: 2, latest: 'Báo cáo Bản thảo Chương 2', status: 'Đã nộp', date: '12/05/2026', github: 'github.com/user/library-system' },
  { group: 'G02', topic: 'Ứng dụng AI nhận diện hình ảnh', members: 3, latest: 'Báo cáo Bản thảo Chương 1', status: 'Đang chấm điểm', date: '10/05/2026', github: 'github.com/user/ai-image-recognition' },
  { group: 'G05', topic: 'Hệ thống bán hàng trực tuyến', members: 2, latest: 'Báo cáo Bản thảo Chương 3', status: 'Đã nộp', date: '15/05/2026', github: 'github.com/user/ecommerce-system' },
]
...
export default function TeacherStudentsPage() {
  ...
  const filteredTTTN = useMemo(() => TTTN.filter((row) => [row.id, row.name, row.company, row.mentor].some((v) => v.toLowerCase().includes(query.toLowerCase()))), [query])
  const filteredDATN = useMemo(() => DATN.filter((row) => [row.group, row.topic, row.latest, row.status].some((v) => v.toLowerCase().includes(query.toLowerCase()))), [query])
