...
export default function TeacherStudentsPage() {
  ...
  const [selectedNhóm hướng dẫn, setSelectedNhóm hướng dẫn] = useState(Nhóm hướng dẫn[0])
  const [selectedNhóm phản biện, setSelectedNhóm phản biện] = useState(Nhóm phản biện[0])

  const filteredNhóm hướng dẫn = useMemo(
    () => Nhóm hướng dẫn.filter((row) => [row.id, row.name, row.company, row.mentor].some((value) => value.toLowerCase().includes(query.toLowerCase()))),
    [query]
  )

  const filteredNhóm phản biện = useMemo(
    () => Nhóm phản biện.filter((row) => [row.group, row.topic, row.latest, row.status].some((value) => value.toLowerCase().includes(query.toLowerCase()))),
    [query]
  )
  ...
  const selectedCount = segment === 'Nhóm hướng dẫn' ? filteredTTTN.length : filteredDATN.length
                  ...
                  <tr key={student.id} className={`border-t border-slate-100 transition hover:bg-slate-50/80 ${selectedTTTN.id === student.id ? 'bg-blue-50/50' : ''}`}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <TeacherButton variant="secondary" className="px-3 py-1.5 text-xs" onClick={() => setSelectedTTTN(student)}>
              ...
              <tbody>
                {filteredDATN.map((group) => (
                  <tr key={group.group} className={`border-t border-slate-100 transition hover:bg-slate-50/80 ${selectedDATN.group === group.group ? 'bg-blue-50/50' : ''}`}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <TeacherButton variant="secondary" className="px-3 py-1.5 text-xs" onClick={() => setSelectedDATN(group)}>
                          <span className="inline-flex items-center gap-1">
                            <Eye className="h-4 w-4" />
                            Chi tiết
                          </span>
