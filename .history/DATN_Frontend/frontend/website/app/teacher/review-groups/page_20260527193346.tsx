...
type ReviewApiResponse = {
  Nhóm hướng dẫnGroups?: Nhóm hướng dẫnGroup[]
...
const defaultNhóm hướng dẫnGroups: Nhóm hướng dẫnGroup[] = []
...
export default function TeacherReviewGroupsPage() {
  ...
  const [Nhóm hướng dẫnGroups, setNhóm hướng dẫnGroups] = useState<Nhóm hướng dẫnGroup[]>(defaultNhóm hướng dẫnGroups)
  const [datnGroups, setDatnGroups] = useState<DatnGroup[]>(defaultDatnGroups)
  const [selectedNhóm hướng dẫnId, setSelectedNhóm hướng dẫnId] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    fetch('/api/mock/teacher/review-groups')
      ...
      .then((data: ReviewApiResponse | null) => {
        const nextNhóm hướng dẫn = data?.Nhóm hướng dẫnGroups ?? []
        const nextDatn = data?.datnGroups ?? []
        setNhóm hướng dẫnGroups(nextNhóm hướng dẫn)
        setDatnGroups(nextDatn)
        setSelectedNhóm hướng dẫnId((current) => current ?? nextNhóm hướng dẫn[0]?.id ?? null)
        setSelectedDatnId((current) => current ?? nextDatn[0]?.id ?? null)
      })
      .finally(() => setLoading(false))
  }, [])

  const filteredNhóm hướng dẫn = useMemo(() => {
    const search = query.toLowerCase()
    return Nhóm hướng dẫnGroups.filter((group) => {
  ...
  }, [query, statusFilter, Nhóm hướng dẫnGroups])
  ...
  const selectedNhóm hướng dẫn = filteredNhóm hướng dẫn.find((group) => group.id === selectedNhóm hướng dẫnId) ?? filteredNhóm hướng dẫn[0] ?? null
  const selectedDatn = filteredDatn.find((group) => group.id === selectedDatnId) ?? filteredDatn[0] ?? null

  const totalCount = Nhóm hướng dẫnGroups.length + datnGroups.length
  const reviewedCount = [...Nhóm hướng dẫnGroups, ...datnGroups].filter((group) => group.status === 'reviewed').length
  const pendingCount = [...Nhóm hướng dẫnGroups, ...datnGroups].filter((group) => group.status === 'pending').length
  const passCount = [...Nhóm hướng dẫnGroups, ...datnGroups].filter((group) => group.evaluation === 'dat').length

  const handleEvaluationChange = (segmentType: Segment, groupId: string, evaluation: EvaluationValue) => {
    if (segmentType === 'Nhóm hướng dẫn') {
      setNhóm hướng dẫnGroups((current) => current.map((group) => (group.id === groupId ? { ...group, evaluation } : group)))
      return
    }
  ...
  const handleSave = async (segmentType: Segment, groupId: string, evaluation: EvaluationValue) => {
    ...
    try {
      ...
      const data = (await response.json()) as { group?: Nhóm hướng dẫnGroup | DatnGroup }
      if (!data.group) return

      if (segmentType === 'Nhóm hướng dẫn') {
        setNhóm hướng dẫnGroups((current) => current.map((group) => (group.id === groupId ? (data.group as Nhóm hướng dẫnGroup) : group)))
      } else {
  ...
  return (
    <>
      <TeacherSectionHeader
        ...
        actions={(
          <>
            <TeacherPill tone="blue">Nhóm hướng dẫn: {Nhóm hướng dẫnGroups.length}</TeacherPill>
            <TeacherPill tone="green">Nhóm phản biện: {datnGroups.length}</TeacherPill>
          </>
        )}
      />
      ...
      <div className="mb-5 grid gap-4 md:grid-cols-4">
        <TeacherStatCard title="Nhóm hướng dẫn" value={`${Nhóm hướng dẫnGroups.length}`} hint="Nhóm thực tập do giảng viên quản lý" accent="blue" />
        <TeacherStatCard title="Nhóm phản biện" value={`${datnGroups.length}`} hint="Nhóm đồ án đang theo dõi" accent="green" />
      ...
      <section className="mb-5 flex flex-wrap gap-2 rounded-[28px] border border-slate-200 bg-white p-2 shadow-[0_12px_40px_rgba(15,23,42,0.05)]">
        {([
          { key: 'Nhóm hướng dẫn', label: 'Nhóm hướng dẫn', count: Nhóm hướng dẫnGroups.length, icon: ShieldCheck },
          { key: 'Nhóm phản biện', label: 'Nhóm phản biện', count: datnGroups.length, icon: ClipboardCheck },
        ] as const).map((item) => {
        ...
        <div className="ml-auto w-full max-w-md">
          <TeacherToolbar placeholder="Tìm nhóm, đề tài, mentor, repo..." value={query} onChange={setQuery}>
            <TeacherPill tone="blue">{segment === 'Nhóm hướng dẫn' ? `${filteredNhóm hướng dẫn.length} nhóm` : `${filteredDatn.length} nhóm`}</TeacherPill>
      ...
      <section className="mb-5 flex flex-wrap gap-2 rounded-[28px] border border-slate-200 bg-white p-2 shadow-[0_12px_40px_rgba(15,23,42,0.05)]">
        {([
          { key: 'all', label: 'Tất cả', count: segment === 'Nhóm hướng dẫn' ? Nhóm hướng dẫnGroups.length : datnGroups.length },
          { key: 'pending', label: 'Chờ đánh giá', count: segment === 'Nhóm hướng dẫn' ? Nhóm hướng dẫnGroups.filter((group) => group.status === 'pending').length : datnGroups.filter((group) => group.status === 'pending').length },
          { key: 'reviewed', label: 'Đã đánh giá', count: segment === 'Nhóm hướng dẫn' ? Nhóm hướng dẫnGroups.filter((group) => group.status === 'reviewed').length : datnGroups.filter((group) => group.status === 'reviewed').length },
        ] as const).map((tab) => {
      ...
      {segment === 'Nhóm hướng dẫn' ? (
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.85fr)]">
          <TeacherCard className="min-w-0">
            <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-4">
              <div>
                <div className="text-sm font-semibold text-slate-900">Nhóm Nhóm hướng dẫn</div>
                <div className="text-xs text-slate-500">Mỗi nhóm chỉ hiện dữ liệu thuộc giảng viên đang quản lý</div>
              </div>
              <TeacherPill tone="orange">{filteredNhóm hướng dẫn.length} nhóm</TeacherPill>
            </div>
            ...
            <div className="overflow-x-auto">
              ...
              <tbody>
                {filteredNhóm hướng dẫn.map((group) => {
                  const selected = selectedNhóm hướng dẫnId === group.id
                  return (
                    <tr key={group.id} className={`border-t border-slate-100 transition hover:bg-slate-50/80 ${selected ? 'bg-blue-50/60' : ''}`}>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex justify-end gap-2">
                          <TeacherButton variant="secondary" className="px-3 py-1.5 text-xs" onClick={() => setSelectedNhóm hướng dẫnId(group.id)}>
          ...
          <TeacherCard className="space-y-4 p-5">
            <div className="text-sm font-semibold text-slate-900">Xem nhanh nhóm Nhóm hướng dẫn</div>
            {selectedNhóm hướng dẫn ? (
              <>
                <div className="rounded-[24px] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_100%)] p-4">
                  ...
                  <div className="mt-1 text-lg font-semibold text-slate-900">{selectedNhóm hướng dẫn.groupName}</div>
                  <div className="mt-1 text-sm text-slate-600">{selectedNhóm hướng dẫn.leader}</div>
                  <div className="mt-4 space-y-2 text-sm text-slate-600">
                    <div className="flex items-center gap-2"><Users className="h-4 w-4 text-[#1976D2]" /> {selectedNhóm hướng dẫn.company}</div>
                    <div className="flex items-center gap-2"><FileText className="h-4 w-4 text-[#1976D2]" /> {selectedNhóm hướng dẫn.latestReport}</div>
                    <div className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-[#1976D2]" /> Cập nhật: {selectedNhóm hướng dẫn.updatedAt}</div>
                  </div>
                </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.85fr)]">
          <TeacherCard className="min-w-0">
            <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-4">
              <div>
                <div className="text-sm font-semibold text-slate-900">Nhóm Nhóm phản biện</div>
          ...
          <TeacherCard className="space-y-4 bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_100%)] p-5">
            <div className="text-sm font-semibold text-slate-900">Xem nhanh nhóm Nhóm phản biện</div>
            {selectedDatn ? (
              <>
                <div className="rounded-[24px] bg-white/85 p-4 ring-1 ring-slate-200">
                  <div className="text-xs text-slate-500">Đang chọn</div>
