...
import { BaseListParams, ListResponseTypeObject } from '../../types/GeneralType';
...
/**
 * Generic parameters:
 * - `TList`   : row shape returned by the list API (used for `columns`, table data, delete).
 * - `TDetail` : full detail shape returned by the detail API (used for modal form initial values).
 * - `TCreate` : request body shape for the create mutation.
 * - `TUpdate` : request body shape for the update mutation.
 *
 * Defaults let simple use-cases keep passing a single type: `<FilterTable<User>>`.
 */
...
> {
  title?: string;
  /** Page-level title shown above the card (bold, large) */
  pageTitle?: string;
  /** Page-level subtitle shown below pageTitle */
  pageSubtitle?: string;
  /** Custom label for the create button (default: "{add_new_btn} {title}") */
  ...
  /** format input: detail API data -> Form values */
  formatInitialValues?: (data: TDetail) => Record<string, unknown>;
  /** format Form values -> create/update request body */
  ...
  return (
    <>
        ...
        <Flex vertical gap={24} align="stretch">
          ...
          <Card className="filter-table-card">
            <div className="filter-table-header flex flex-col gap-4 mb-4">
              {/** Table Title and Button Add New */}
              <Flex justify="space-between" align="center">
                {title && (
                ...
                {/* Always show create button inside card (Figma) */}
                <Flex align="center" gap={12}>
                  {renderCreateButton()}
                  {exportInfo?.type && exportMutation && (
                    <Button
      ...
      >
        ...
        >
          {/*
            Render exactly one modal content based on the current openModal.type.
            Update / Detail variants are cloned with the fetched `detail` so the
            child component can read it without having to pull from the query cache itself.
          */}
          {openModal.type === CreateModalType &&
