import { TeamOutlined, SearchOutlined, DownloadOutlined, SendOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
import { Button, Card, Input, Modal, Pagination, Space, Tag, Typography, message, Tabs, Checkbox, Radio, Select } from 'antd';
        ...
        <div className="grid grid-cols-3 gap-4 px-4 pb-4">
          ...
          <div className="col-span-1">
            <Card className="rounded-[12px]">
              ...
              <Input allowClear prefix={<SearchOutlined />} placeholder="Tìm tên GV..." className="mb-3" />
          ...
          <Card>
            <div className="flex items-start gap-4 justify-between mb-4">
              <div className="flex-1 space-y-3">
                <Select defaultValue="all" style={{ width: 220 }}>
                  ...
                  <Select.Option value="ktpm">KTPM2020</Select.Option>
                  <Select.Option value="cnpm">CNPM2020</Select.Option>
                </Select>
                <Select defaultValue="all" style={{ width: 220 }}>
                ...
                <Input allowClear placeholder="MSSV / tên SV..." style={{ width: 360 }} />
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.filter((r) => r.status === 'assigned').map((r) => (
                    <tr key={r.studentId} className="border-t border-slate-100 hover:bg-slate-50">
                      <td className="px-4 py-3 text-[#2563eb] font-medium">{r.studentId}</td>
                      <td className="px-4 py-3">{r.name}</td>
                      <td className="px-4 py-3 text-slate-600">{r.className}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => {
                            Modal.confirm({
                              title: 'Chỉnh sửa GVHD',
                              content: (
                                <div className="flex flex-col gap-2">
                                  <Input defaultValue={r.supervisor || ''} placeholder="Tên giảng viên" id={`edit-${r.studentId}`} />
                                </div>
                              ),
                              okText: 'Lưu',
                              cancelText: t(getKey('cancel_btn')),
                              onOk: () => {
                                const el = document.getElementById(`edit-${r.studentId}`) as HTMLInputElement | null;
                                if (el) {
                                  const val = el.value.trim();
                                  if (!val) return Promise.reject();
                                  setRows((prev) => prev.map((row) => (row.studentId === r.studentId ? { ...row, supervisor: val } : row)));
                                  message.success('Đã cập nhật GVHD');
                                }
                              },
                              centered: true,
                            });
                          }} className="text-[#2563eb]"> <EditOutlined /> </button>
                          <button onClick={() => unassign(r.studentId)} className="text-[#F44336]"> <DeleteOutlined /> </button>
                        </div>
                      </td>
                    </tr>
                  ))}
