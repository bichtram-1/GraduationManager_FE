...
const CompaniesPage = () => {
  ...
  const columns = useMemo<ColumnsType<CompanyRow>>(() => [
    ...
    },
    {
      ...
      render: () => (
        <Space size={4}>
          <Button type="text" size="small" className="!px-2">Xem</Button>
          <Button type="text" size="small" className="!px-2">Sửa</Button>
          <Button type="text" size="small" danger className="!px-2">Xóa</Button>
        </Space>
      ),
    },
  ], [t]);
      ...
      <Card className="overflow-hidden rounded-[18px] border border-slate-100 shadow-[0_12px_28px_rgba(15,23,42,0.05)]">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Space wrap size={8}>
            ...
            <Button type="primary" icon={<PlusOutlined className="text-white" />} onClick={() => setOpen(true)} className="!h-10 !rounded-[8px] !bg-primary !px-5 !flex !items-center !gap-2 !font-medium hover:!bg-blueDark">
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 xl:grid-cols-12">
          ...
          <div className="xl:col-span-3">
            <Select className="!h-11 !w-full" placeholder={t(getKey('company_field'))} options={[{ value: 'all', label: 'Tất cả lĩnh vực' }, { value: 'software', label: 'Phần mềm' }, { value: 'ecommerce', label: 'Thương mại điện tử' }, { value: 'outsourcing', label: 'Gia công phần mềm' }]} />
          </div>
          <div className="xl:col-span-4">
            <Select className="!h-11 !w-full" placeholder={t(getKey('company_status'))} options={[{ value: 'all', label: 'Tất cả trạng thái' }, { value: 'active', label: t(getKey('company_active')) }, { value: 'pending', label: t(getKey('company_pending')) }, { value: 'paused', label: t(getKey('company_paused')) }]} />
          </div>
        </div>
      ...
      >
        <Form form={form} layout="vertical" className="max-h-[75vh] overflow-y-auto pr-1">
          <div className="grid grid-cols-1 gap-x-5 gap-y-4 md:grid-cols-2">
            <Form.Item label={t(getKey('company_name'))} required>
              <Input placeholder="VD: FPT Software" />
            </Form.Item>
            <Form.Item label={t(getKey('company_tax_id'))} required>
              <Input placeholder="VD: 0101243150" />
            </Form.Item>
            <Form.Item label={t(getKey('company_field'))} required>
              <Select options={[{ value: 'software', label: 'Phần mềm & công nghệ' }, { value: 'ecommerce', label: 'Thương mại điện tử' }, { value: 'outsourcing', label: 'Gia công phần mềm' }]} />
            </Form.Item>
            <Form.Item label={t(getKey('company_status'))} required>
              <Select options={[{ value: 'active', label: t(getKey('company_active')) }, { value: 'pending', label: t(getKey('company_pending')) }, { value: 'paused', label: t(getKey('company_paused')) }]} />
            </Form.Item>
            <Form.Item label={t(getKey('company_contact'))} required>
              <Input placeholder="VD: Nguyễn Văn Hùng" />
            </Form.Item>
            <Form.Item label="Số điện thoại" required>
              <Input placeholder="0901234567" />
            </Form.Item>
            <Form.Item label={t(getKey('email'))} required>
              <Input placeholder="contact@company.com" />
            </Form.Item>
            <Form.Item label="Số sinh viên đang hợp tác">
